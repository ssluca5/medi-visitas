...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    async function handleSalvarProfissional() {
      ...
      const apiData = {
        ...
        dataNascimento: formDataNascimento
          ? new Date(formDataNascimento).toISOString()
          : void 0,
        ...
        dataNascConjuge: formDataNascConjuge
          ? new Date(formDataNascConjuge).toISOString()
          : void 0,
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const msg =
            errBody?.error || errBody?.message || `Erro ${response.status}`;
          throw new Error(msg);
        }
    ...
    function $$render_inner($$renderer3) {
      ...
      Users($$renderer3, { class: "h-4.5 w-4.5 text-white" });
      $$renderer3.push(
        `<!----></div> <div><h1 class="text-lg font-bold text-slate-800">Profissionais</h1> <p class="text-[11px] text-slate-400">Gerencie o cadastro e a classificação dos médicos</p></div></div> <div class="flex items-center gap-2">`,
      );
      Button($$renderer3, {
      ...
      $$renderer3.push(
        `<!----></div></div> <div class="card-surface p-4 mb-6"><div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4"><div class="relative">`,
      );
      Search($$renderer3, {
        class:
      ...
      $$renderer3.push(
        `<!----> <input type="text" placeholder="Buscar por nome ou CRM..."${attr("value", filtroBusca)} class="input-base !pl-9"/></div> `,
      );
      $$renderer3.select(
        {
      ...
      if (loading) {
        ...
        $$renderer3.push(
          `<!----></div> <div class="text-center"><p class="text-sm font-medium text-slate-700">Nenhum profissional encontrado</p> <p class="text-xs text-slate-400 mt-1">Use os filtros acima ou adicione um novo</p></div> `,
        );
        Button($$renderer3, {
          size: "sm",
          onclick: handleNovoProfissional,
        ...
        for (
          ...
          $$renderer3.push(
            `<!----></button> <a${attr("href", `/dashboard/profissionais/${prof.id}`)} title="Agenda / Visitas" class="p-2 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-slate-100 transition-all duration-200 cursor-pointer">`,
          );
          Calendar($$renderer3, { class: "w-3.5 h-3.5" });
          $$renderer3.push(
            `<!----></a> <button${attr("disabled", prof.estagioPipeline === "PROSPECTADO" || !isAtivo, true)} title="Retroceder estágio" class="p-2 rounded-lg text-slate-500 opacity-60 hover:opacity-100 hover:text-slate-800 hover:bg-slate-100 transition-all duration-200 cursor-pointer disabled:opacity-20 disabled:cursor-not-allowed">`,
          );
          Arrow_left($$renderer3, { class: "w-3.5 h-3.5" });
          $$renderer3.push(
            `<!----></button> <button${attr("disabled", prof.estagioPipeline === "FIDELIZADO" || !isAtivo, true)} title="Avançar estágio" class="p-2 rounded-lg text-slate-500 opacity-60 hover:opacity-100 hover:text-slate-800 hover:bg-slate-100 transition-all duration-200 cursor-pointer disabled:opacity-20 disabled:cursor-not-allowed">`,
          );
          Arrow_right($$renderer3, { class: "w-3.5 h-3.5" });
          $$renderer3.push(
            `<!----></button> <button${attr("title", isAtivo ? "Inativar" : "Ativar")}${attr_class(`p-2 rounded-lg text-slate-500 opacity-60 hover:opacity-100 hover:bg-slate-100 transition-all duration-200 cursor-pointer ${stringify(isAtivo ? "hover:text-amber-600" : "hover:text-green-600")}`)}>`,
          );
          if (isAtivo) {
            $$renderer3.push("<!--[0-->");
        ...
        if (pagination.totalPages > 1) {
          $$renderer3.push("<!--[0-->");
          $$renderer3.push(
            `<div class="mt-4 flex items-center justify-between"><p class="text-xs text-slate-400">Página ${escape_html(pagination.page)} de ${escape_html(pagination.totalPages)} · ${escape_html(pagination.total)} total</p> <div class="flex gap-1.5">`,
          );
          Button($$renderer3, {
            variant: "outline",
            size: "sm",
      ...
      $$renderer3.push(`<!--]--> `);
      {
        let children = function ($$renderer4) {
          ...
          $$renderer4.push(
            `<!----> Dados Básicos</h4> <div class="space-y-3"><div class="grid gap-3" style="grid-template-columns: 30% 1fr"><div><label for="prof-tratamento" class="input-label">Tratamento</label> `,
          );
          $$renderer4.select(
            {
              id: "prof-tratamento",
          ...
          if (formContatos.length === 0) {
            ...
            for (
              let idx = 0, $$length = each_array_2.length;
              idx < $$length;
              idx++
            ) {
              ...
              $$renderer4.push(
                `<!----></button> <div class="space-y-2"><div class="w-full"><label${attr("for", `contato-tipo-${stringify(idx)}`)} class="input-label text-[10px]">Tipo</label> `,
              );
              $$renderer4.select(
                {
          ...
          $$renderer4.push(
            `<!--]--></div></div></section> <section class="border-t border-slate-100 pt-6 mt-6"><h4 class="section-header">`,
          );
          Map_pin($$renderer4, { class: "h-3.5 w-3.5" });
          $$renderer4.push(
            `<!----> Atuação</h4> <div class="space-y-3"><div><label for="prof-esp" class="input-label">Especialidade</label> `,
          );
          $$renderer4.select(
            {
              id: "prof-esp",
          ...
          if (formEspecialidadeId && subEspecialidades.length > 0) {
            $$renderer4.push("<!--[0-->");
            $$renderer4.push(
              `<div><label for="prof-sub" class="input-label">Subespecialidade</label> `,
            );
            $$renderer4.select(
              {
                id: "prof-sub",
          ...
          $$renderer4.push(
            `<!--]--> <div><label for="prof-cep" class="input-label">CEP</label> <div class="relative max-w-xs"><input id="prof-cep" type="text"${attr("value", formCep)} class="input-base !pr-8" placeholder="01001-000"${attr("maxlength", 9)}/> `,
          );
          {
            $$renderer4.push("<!--[-1-->");
          }
          $$renderer4.push(
            `<!--]--></div> <p class="text-xs text-slate-400 mt-1">Preenche endereço, bairro, cidade e UF</p></div> <div><label for="prof-logradouro" class="input-label">Endereço</label> <textarea id="prof-logradouro"${attr("rows", 2)} class="input-base resize-none leading-relaxed" placeholder="Rua, Av., Alameda...">`,
          );
          ...
          for (
            ...
            $$renderer4.push(
              `<button type="button"${attr_class(`segmented-btn ${stringify(formPotencial === opt.value ? opt.active : "")}`)}>${escape_html(opt.label)}</button>`,
            );
          }
          $$renderer4.push(
            `<!--]--></div> <p class="text-xs text-slate-400 mt-1.5">Volume estimado de prescrições</p></div> <div class="mt-5"><span class="input-label">Estágio no pipeline</span> <div class="segmented-control" role="group" aria-label="Estágio no pipeline"><!--[-->`,
          );
          ...
          Button($$renderer4, {
            ...
            children: ($$renderer5) => {
              $$renderer5.push(
                `<!---->${escape_html(profissionalEmEdicao ? "Salvar Alterações" : "Cadastrar Profissional")}`,
              );
            },
            $$slots: { default: true },
          });
      ...
      $$renderer3.push(`<!----> `);
      {
        let description = function ($$renderer4) {
          if (profToDelete) {
            $$renderer4.push("<!--[0-->");
            $$renderer4.push(
              `<p>Você está prestes a excluir <strong>"${escape_html(profToDelete.nome)}"</strong>.</p> <p>Esta ação não pode ser desfeita.</p>`,
            );
          } else {
            $$renderer4.push("<!--[-1-->");
          }
