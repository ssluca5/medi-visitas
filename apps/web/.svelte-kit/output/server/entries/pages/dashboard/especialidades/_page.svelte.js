import { s as sanitize_props, a as spread_props, b as slot, l as head, f as escape_html, h as attr, d as stringify, e as ensure_array_like, i as attr_class, k as derived } from "../../../../chunks/index.js";
  ...
  $$renderer.component(($$renderer2) => {
    ...
    async function fetchEspecialidades() {
      ...
      try {
        ...
        const withSubs = await Promise.all(json.data.map(async (esp) => {
          try {
    ...
    async function handleSalvarEspecialidade() {
      ...
      try {
        const response = await apiFetch(url, data.sessionToken, { method, body: JSON.stringify(formData) });
        if (!response.ok) throw new Error(`Erro ${response.status}`);
        sheetOpen = false;
    ...
    function $$render_inner($$renderer3) {
      ...
      $$renderer3.push(`<header class="mb-6 flex items-center justify-between"><div><h2 class="text-2xl font-semibold tracking-tight text-slate-900">Especialidades</h2> <p class="text-sm text-slate-400 mt-1">${escape_html(especialidades.length)} especialidade(s) cadastrada(s)</p></div> `);
      Button($$renderer3, {
      ...
      if (loading) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="card-surface flex items-center justify-center py-20"><div class="flex flex-col items-center gap-3"><div class="h-8 w-8 animate-spin rounded-full border-2 border-slate-200 border-t-blue-600"></div> <span class="text-sm text-slate-400">Carregando especialidades...</span></div></div>`);
      } else if (error) {
        $$renderer3.push("<!--[1-->");
        ...
        $$renderer3.push(`<!----></div> <div class="text-center"><p class="text-sm font-medium text-slate-700">Nenhuma especialidade cadastrada</p> <p class="text-xs text-slate-400 mt-1">Comece adicionando a primeira especialidade</p></div> `);
        Button($$renderer3, {
          size: "sm",
          onclick: handleNovaEspecialidade,
          class: "gap-2",
        ...
        for (let $$index_2 = 0, $$length = each_array.length; $$index_2 < $$length; $$index_2++) {
          ...
          for (let $$index_1 = 0, $$length2 = each_array_1.length; $$index_1 < $$length2; $$index_1++) {
            ...
            $$renderer3.push(`<tr${attr_class("group border-t border-slate-50 transition-all duration-200 cursor-pointer hover:bg-slate-50/60", void 0, { "opacity-50": !esp.ativo })}><td class="p-3.5">`);
            if (subCount > 0) {
              $$renderer3.push("<!--[0-->");
              $$renderer3.push(`<button class="p-1 rounded-md hover:bg-slate-100 transition-all duration-200 cursor-pointer"${attr("title", isExpanded ? "Recolher" : "Expandir")}>`);
              if (isExpanded) {
                $$renderer3.push("<!--[0-->");
            ...
            $$renderer3.push(`<!--]--></td><td class="p-3.5"><span${attr_class("text-sm font-medium text-slate-800", void 0, { "text-slate-400": !esp.ativo })}>${escape_html(esp.nome)}</span></td><td class="p-3.5 text-center">`);
            if (subCount > 0) {
              $$renderer3.push("<!--[0-->");
              $$renderer3.push(`<span class="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-slate-100 px-1.5 text-[11px] font-medium text-slate-600">${escape_html(subCount)}</span>`);
            } else {
              $$renderer3.push("<!--[-1-->");
              $$renderer3.push(`<span class="text-xs text-slate-300">—</span>`);
            }
            $$renderer3.push(`<!--]--></td><td class="p-3.5"><span${attr_class("inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium", void 0, {
              "bg-emerald-50": esp.ativo,
              "text-emerald-700": esp.ativo,
              "bg-slate-100": !esp.ativo,
              "text-slate-400": !esp.ativo
            })}>${escape_html(esp.ativo ? "Ativa" : "Inativa")}</span></td><td class="p-3.5"><div class="flex justify-center items-center gap-0.5"><button${attr("title", esp.ativo ? "Inativar" : "Ativar")}${attr_class(`p-2 rounded-lg opacity-60 hover:opacity-100 transition-all duration-200 cursor-pointer ${stringify(esp.ativo ? "hover:text-amber-600 hover:bg-amber-50" : "hover:text-green-600 hover:bg-green-50")}`)}>`);
            if (esp.ativo) {
              $$renderer3.push("<!--[0-->");
            ...
            $$renderer3.push(`<!--]--></button> <button title="Excluir" class="p-2 rounded-lg opacity-60 hover:opacity-100 hover:bg-red-50 transition-all duration-200 cursor-pointer">`);
            Trash_2($$renderer3, {
              class: "w-3.5 h-3.5 text-slate-400 hover:text-red-500 transition-colors"
            });
            ...
            if (isExpanded) {
              ...
              for (let $$index = 0, $$length3 = each_array_2.length; $$index < $$length3; $$index++) {
                ...
                $$renderer3.push(`<tr${attr_class("border-t border-slate-50 bg-slate-25 transition-all", void 0, { "opacity-60": !!sub.deletedAt, "italic": !!sub.deletedAt })}><td class="p-3.5"></td><td class="p-3.5 pl-9"><span class="text-sm text-slate-500">↳ ${escape_html(sub.nome)}</span></td><td class="p-3.5"></td><td class="p-3.5">`);
                if (sub.deletedAt) {
                  $$renderer3.push("<!--[0-->");
                  $$renderer3.push(`<span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium bg-slate-100 text-slate-400">Inativa</span>`);
                } else {
                  $$renderer3.push("<!--[-1-->");
                  $$renderer3.push(`<span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium bg-emerald-50 text-emerald-700">Ativa</span>`);
                }
                $$renderer3.push(`<!--]--></td><td class="p-3.5"><div class="flex justify-center items-center gap-0.5"><button${attr("title", sub.deletedAt ? "Ativar" : "Inativar")}${attr_class(`p-2 rounded-lg opacity-40 hover:opacity-100 transition-all duration-200 cursor-pointer ${stringify(sub.deletedAt ? "hover:text-green-600 hover:bg-green-50" : "hover:text-amber-600 hover:bg-amber-50")}`)}>`);
                if (sub.deletedAt) {
                  $$renderer3.push("<!--[0-->");
                  Play($$renderer3, { class: "w-4 h-4" });
                } else {
                ...
                $$renderer3.push(`<!--]--></button> <button title="Excluir subespecialidade" class="p-2 rounded-lg opacity-40 hover:opacity-100 hover:bg-red-50 transition-all duration-200 cursor-pointer">`);
                Trash_2($$renderer3, {
                  class: "w-3 h-3 text-slate-400 hover:text-red-500 transition-colors"
                });
                $$renderer3.push(`<!----></button></div></td></tr>`);
              }
              $$renderer3.push(`<!--]--> `);
      ...
      $$renderer3.push(`<!--]--> `);
      {
        let children = function($$renderer4) {
          ...
          Button($$renderer4, {
            ...
            children: ($$renderer5) => {
              $$renderer5.push(`<!---->${escape_html(especialidadeEmEdicao ? "Salvar" : "Criar")}`);
            },
            $$slots: { default: true }
          });
