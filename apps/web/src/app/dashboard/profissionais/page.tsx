...
import {
  ...
  FiltrosProfissionaisState,
} from "@/components/profissionais/FiltrosProfissionais";
import {
  ProfissionalSheet,
  ProfissionalFormData,
} from "@/components/profissionais/ProfissionalSheet";
...
export default function ProfissionaisPage() {
  ...
  const [profissionalEmEdicao, setProfissionalEmEdicao] =
    useState<ProfissionalFormData | null>(null);
  const [especialidades, setEspecialidades] = useState<
    Array<{ id: string; nome: string; categoria: string }>
  >([]);
  ...
  const fetchProfissionais = useCallback(
    async (page: number = 1) => {
      ...
      try {
        ...
        if (filtros.estagioPipeline)
          params.append("estagioPipeline", filtros.estagioPipeline);

        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/profissionais?${params}`,
          {
            headers: {
  ...
  const handleSalvarProfissional = async (data: ProfissionalFormData) => {
    ...
    const apiData = {
      ...
      endereco: Object.values(data.endereco).some((v) => v)
        ? data.endereco
        : undefined,
  ...
  const handleAvancarEstagio = async (profissional: Profissional) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/profissionais/${profissional.id}/estagio`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleToggleAtivo = async (profissional: Profissional) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/profissionais/${profissional.id}/ativo`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleExcluirProfissional = async (profissional: Profissional) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/profissionais/${profissional.id}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        <nav className="flex-1 space-y-1">
          {navItems.map((item) => (
            <Link
              ...
              style={{
                color:
                  item.href === "/dashboard/profissionais"
                ...
                <tbody>
                  {profissionais.map((profissional) => {
                    ...
                    return (
                      ...
                      >
                        <td
                          className="p-4 w-1/3 text-left"
                          onClick={() => handleEditarProfissional(profissional)}
                        >
                          <div>
                            <div
                              className={`font-medium ${isAtivo ? "text-slate-900" : "text-slate-400"}`}
                            >
                              {profissional.nome}
                            </div>
                            <div
                              className={`text-sm ${isAtivo ? "text-slate-500" : "text-slate-300"}`}
                            >
                        ...
                        <td
                          className="p-4 text-sm w-1/4 text-left"
                          onClick={() => handleEditarProfissional(profissional)}
                        >
                        ...
                        <td className="p-4 w-28 text-center">
                          <EstagioBadge
                            estagio={profissional.estagioPipeline}
                          />
                        </td>
