<script lang="ts">
	...
	const potencialColors: Record<PotencialPrescricao, string> = {
		...
		BAIXO: 'bg-[rgb(var(--slate-50))] text-ui-secondary ring-1 ring-slate-200'
	};
...
<div class="flex flex-col h-full">
	<div class="flex items-center gap-2 mb-4">
		...
		<div>
			<h3 class="text-sm font-semibold text-ui-strong">Sugestões</h3>
			<p class="text-[10px] text-ui-muted">Quem você deve visitar</p>
		</div>
	</div>
	...
	{:else if sugestoes.length === 0}
		<div class="text-center py-8">
			<p class="text-sm text-ui-muted">Nenhuma sugestão disponível</p>
		</div>
	{:else}
				...
				>
					<div class="flex items-start justify-between gap-2">
						<div class="min-w-0 flex-1">
							<p
								class="text-[13px] font-semibold text-ui-body truncate group-hover-text-ui-primary transition-colors"
							>
								{sug.profissional.nome}
							</p>
							{#if sug.profissional.especialidade}
								<p class="text-[10px] text-ui-muted mt-0.5 truncate">
									{sug.profissional.especialidade.nome}
								</p>
							{/if}
						</div>
						<div
							class="flex items-center gap-1 rounded-md px-1.5 py-0.5 bg-[rgb(var(--slate-50))] text-[10px] font-bold text-ui-secondary"
					...
					<div class="mt-2 flex items-center gap-3">
						...
						<span class="flex items-center gap-1 text-[10px] text-ui-muted">
