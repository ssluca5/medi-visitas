<script lang="ts">
  ...
  function potencialBadge(potencial: string) {
    switch (potencial) {
      case 'ALTO': return 'bg-emerald-100 text-ui-success-strong';
      case 'MEDIO': return 'bg-amber-100 text-ui-warning-strong';
      case 'BAIXO': return 'bg-orange-100 text-ui-warning-strong';
      case 'ESTRATEGICO': return 'bg-indigo-100 text-indigo-800';
      default: return 'bg-[rgb(var(--slate-100))] text-ui-secondary';
    }
  }
...
<div class="mb-4">
  <div class="relative">
    ...
    <input
      ...
      class="w-full pl-10 pr-8 h-9 rounded-lg border border-[rgb(var(--slate-200))] bg-white text-sm text-ui-body placeholder-ui-muted focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500"
    />
    {#if busca}
      <button
      ...
      >
        <X class="h-3.5 w-3.5 text-ui-muted" />
      </button>
    {/if}
  </div>
</div>
    ...
    >
      ...
      <div class="h-10 flex items-center justify-between px-3 mb-2 flex-shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[12px] font-semibold text-ui-body">{col.label}</span>
          <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-md bg-[rgb(var(--slate-100))] text-ui-secondary">{total}</span>
        </div>
      </div>
          ...
          {#each profissionais as prof}
            ...
            >
              ...
              <div class="flex items-start gap-1.5">
                <GripVertical class="h-3.5 w-3.5 text-ui-disabled mt-0.5 shrink-0" />
                <p class="text-[13px] font-medium text-ui-strong leading-snug min-w-0 flex-1">{prof.nome}</p>
              </div>

              <!-- Linha 2: Especialidade + Badge Potencial -->
              <div class="flex items-center justify-between mt-2 gap-1 pl-5">
                <p class="text-[10px] text-ui-muted truncate flex-1">
              ...
              <!-- Linha 3: CRM -->
              <p class="text-[10px] text-ui-muted mt-1 pl-5">
                CRM {prof.crm ?? '—'}
              </p>
            </div>
          {/each}
