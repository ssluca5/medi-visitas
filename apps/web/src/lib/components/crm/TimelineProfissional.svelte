      ...
      <div class="relative">
        ...
        {#if item.tipo === 'VISITA'}
          <div class="absolute -left-[calc(1.5rem+5px)] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full ring-[3px] ring-[rgb(var(--slate-50))] shadow-sm
            {item.status === 'REALIZADA' ? 'bg-emerald-500' : item.status === 'CANCELADA' ? 'bg-red-500' : item.status === 'NAO_REALIZADA' ? 'bg-[rgb(var(--slate-50))]0' : 'bg-blue-500'}
          "></div>
        {:else if item.tipo === 'ESTAGIO'}
          <div class="absolute -left-[calc(1.5rem+5px)] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full ring-[3px] ring-[rgb(var(--slate-50))] shadow-sm bg-violet-500"></div>
        {:else}
          <div class="absolute -left-[calc(1.5rem+5px)] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full ring-[3px] ring-[rgb(var(--slate-50))] shadow-sm bg-amber-500"></div>
        {/if}

        <!-- Card -->
        <div class="bg-white rounded-xl p-4 shadow-sm border border-[rgb(var(--slate-100))] will-change-transform hover:shadow-md transition-all duration-200">
          <!-- Header: icon + date -->
