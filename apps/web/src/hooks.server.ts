...
/**
 * Decodifica JWT localmente sem verificar assinatura.
 * Usado como fallback quando a verificação remota falha por rede.
 */
...
async function getUserFromToken(token: string): Promise<{
  ...
  let result: { userId: string | null; sessionId: string | null; sessionToken: string; userName: string | null } | null = null
  ...
  if (!result && !token.startsWith('eyJ')) {
    try {
      ...
      const session = client.sessions?.find((s: { status: string }) => s.status === 'active')
      if (session) {
...
export const handle: Handle = async ({ event, resolve }) => {
  ...
  const clerkToken = url.searchParams.get('__clerk_db_jwt')
  ...
  if (token) {
    ...
    if (user) {
      ...
      // Fallback: decodificar JWT localmente sem verificar assinatura
      // → Evita deslogar por falha transitória de rede
      if (token.startsWith('eyJ')) {
        ...
        if (payload) {
          ...
          if (!isExpired) {
            // Token válido localmente — manter sessão mesmo sem verificação remota
            locals.userId = (payload.sub as string) ?? null
            locals.sessionId = (payload.sid as string) ?? null
            locals.sessionToken = token
            locals.userName = [payload.firstName, payload.lastName]
              .filter(Boolean).join(' ')
              || (payload.email as string)
              || null
          } else {
            // JWT realmente expirado — deletar cookie
        ...
        // Token opaco sem cache e sem poder verificar — deletar
        cookies.delete(COOKIE_NAME, { path: '/' })
      }
    }
  }
