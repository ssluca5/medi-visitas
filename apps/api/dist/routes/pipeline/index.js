...
import { PipelineQuerySchema, MetricasQuerySchema, EvolucaoQuerySchema, VisitasPeriodoQuerySchema, } from "./schemas.js";
const ESTAGIOS = [
    "PROSPECTADO",
    "VISITADO",
    "INTERESSADO",
...
export async function calcularTaxaConversao(estagioOrigem, estagioDestino, dataInicio, dataFim, prismaClient) {
...
const pipelineRoutes = async (app) => {
    ...
    app.get("/metricas", async (request, reply) => {
        ...
        const taxas = await Promise.all(pares.map(([origem, destino]) => calcularTaxaConversao(origem, destino, dataInicio, dataFim, prisma)));
        // Profissionais sem visita
        const semVisita = await profissionaisSemVisita(30, prisma);
        // Média de visitas por semana
        const semanas = Math.max(1, Math.ceil((dataFim.getTime() - dataInicio.getTime()) / (7 * 24 * 60 * 60 * 1000)));
        const mediaVisitasPorSemana = Math.round((visitasRealizadas / semanas) * 10) / 10;
        return {
            totalProfissionais,
            totalAtivos,
            visitasRealizadas,
    ...
    app.get("/evolucao", async (request, reply) => {
        ...
        for (const log of logs) {
            const key = granularidade === "semana"
        ...
        const data = periodKeys.map((key) => ({
            periodo: key,
            label: granularidade === "semana" ? getWeekLabel(key) : getMonthLabel(key),
            ...contadores[key],
        }));
        return { data };
    });
    ...
    app.get("/exportar", async (request, reply) => {
        ...
        return reply
            ...
            .header("Content-Disposition", `attachment; filename="pipeline-${hoje}.csv"`)
            .send(linhas.join("\n"));
    });
};
