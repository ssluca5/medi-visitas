...
function getPriceId(plano) {
    ...
    if (!priceId)
        throw new Error(`Price ID nao configurado para plano ${plano}`);
    return priceId;
}
const billingRoutes = async (app) => {
    app.addHook("preHandler", verifyClerkToken);
    app.addHook("preHandler", resolveTenant);
    app.post("/checkout", { config: { rateLimit: { max: 5, timeWindow: "1 hour" } } }, async (request, reply) => {
        ...
        try {
            const checkoutUrl = await criarCheckout(org.id, org.stripeCustomerId, body.plano, request.userId, getPriceId(body.plano));
    ...
    app.get("/status", async (request, reply) => {
        ...
        const transcricoesUsadasMes = org.plano !== "TRIAL" && org.transcricoesMes !== mesAtual
            ? 0
            : org.transcricoesUsadas;
        const transcricoesRestantes = Math.max(0, org.transcricoesLimite + org.transcricoesExtras - transcricoesUsadasMes);
        return reply.send({
            plano: org.plano,
            status: org.status,
            diasRestantes,
