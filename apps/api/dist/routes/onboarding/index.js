...
const onboardingRoutes = async (app) => {
    ...
    // POST /individual
    app.post("/individual", { config: { rateLimit: { max: 5, timeWindow: "1 hour" } } }, async (request, reply) => {
        const userId = request.userId;
        if (!userId)
            return reply.status(401).send({ error: "Unauthorized" });
    ...
    // POST /empresa
    app.post("/empresa", { config: { rateLimit: { max: 5, timeWindow: "1 hour" } } }, async (request, reply) => {
        const userId = request.userId;
        if (!userId)
            return reply.status(401).send({ error: "Unauthorized" });
