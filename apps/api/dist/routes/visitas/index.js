...
import { transcreverAudio, extrairCamposVisita, } from "../../services/gemini.js";
import { verificarLimiteTranscricao, incrementarTranscricao, } from "../../services/transcricoes.js";
...
const visitasRoutes = async (app) => {
    ...
    app.patch("/:id/status", async (request, reply) => {
        ...
        if (STATUS_FINAIS.includes(existing.status)) {
            ...
            if (hasPassed) {
                return reply.status(409).send({
                    error: "Não é possível alterar o status de uma visita finalizada que já passou.",
                });
            }
        }
        const visita = await prisma.visita.update({
