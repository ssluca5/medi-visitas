...
import { CreateAgendaItemSchema, UpdateAgendaItemSchema, ListAgendaQuerySchema, VincularVisitaSchema, SugestoesQuerySchema, } from "./schemas.js";
...
const agendaRoutes = async (app) => {
    ...
    app.get("/sugestoes", async (request, reply) => {
        ...
        const sugestoes = profissionais.map((p) => {
            const ultimaVisita = p.visitas.length > 0 ? p.visitas[0].dataVisita : null;
    ...
    app.put("/:id", async (request, reply) => {
        ...
        if (data.dataHoraFim)
            data.dataHoraFim = new Date(data.dataHoraFim);
