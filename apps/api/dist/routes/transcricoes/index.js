...
function getPriceIdPacoteIa(quantidade) {
    const map = {
...
const transcricoesRoutes = async (app) => {
    ...
    app.get("/status", async (request, reply) => {
        const { permitido, usadas, limite, extras } = await verificarLimiteTranscricao(request.organizationId);
        return { usadas, limite, extras, restantes: limite - usadas, permitido };
    });
    app.post("/comprar-pacote", { config: { rateLimit: { max: 5, timeWindow: "1 hour" } } }, async (request, reply) => {
        ...
        const checkoutUrl = await criarCheckoutPacoteIa(request.organizationId, org.stripeCustomerId, body.quantidade, getPriceIdPacoteIa(body.quantidade));
        return { checkoutUrl };
    });
};
