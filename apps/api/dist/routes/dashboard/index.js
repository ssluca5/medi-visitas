...
export function buildAlertas(params) {
    ...
    for (const p of params.profissionaisComVisitas) {
        ...
        if (!ultimaVisita)
            continue;
        const diasSemVisita = Math.floor((params.agora.getTime() - ultimaVisita.getTime()) / (1000 * 60 * 60 * 24));
        if (diasSemVisita > 60) {
            alertas.push({
...
const dashboardRoutes = async (app) => {
    ...
    app.get("/resumo", async (request, reply) => {
        ...
        const [totalProfissionais, totalEspecialidades, visitasHoje, visitasSemana, ultimasVisitas, proximosAgendamentos,] = await Promise.all([
    ...
    app.get("/alertas", async (request, reply) => {
        ...
        const [profissionaisComVisitas, agendaAtrasados, agendaHoje, prospectadosSemVisita,] = await Promise.all([
    ...
    app.get("/gestor", async (request, reply) => {
        ...
        const [totalProfissionais, totalVisitas, funilAgregadoDb, visitasMembros] = await Promise.all([
