...
export default async function subespecialidadesRoutes(app) {
    ...
    app.post("/subespecialidades", {
        ...
        if (existente) {
            return reply.code(409).send({
                error: "Já existe uma subespecialidade com este nome para esta especialidade",
            });
        }
    ...
    // ============================================
    app.patch("/subespecialidades/:id/ativo", {
        preHandler: [verifyClerkToken],
    }, async (request, reply) => {
        const { id } = request.params;
    ...
    // ============================================
    app.delete("/subespecialidades/:id", {
        preHandler: [verifyClerkToken],
    }, async (request, reply) => {
        const { id } = request.params;
        const subespecialidade = await prisma.subEspecialidade.findUnique({
