...
export async function criarCheckout(organizationId, stripeCustomerId, plano, userId, priceId) {
    if (!process.env.STRIPE_SECRET_KEY) {
        throw new Error("Billing não configurado. Adicione STRIPE_SECRET_KEY no .env");
    }
...
export async function criarCheckoutPacoteIa(organizationId, stripeCustomerId, quantidade, priceId) {
    if (!process.env.STRIPE_SECRET_KEY) {
        throw new Error("Billing nao configurado. Adicione STRIPE_SECRET_KEY no .env");
    }
    const session = await stripe.checkout.sessions.create({
        mode: "payment",
        payment_method_types: ["card"],
