...
export async function extrairCamposVisita(transcricao) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), TIMEOUT_MS);
    try {
        const res = await fetch(getUrl(`/models/${GEMINI_MODEL}:generateContent`), {
            ...
            body: JSON.stringify({
                system_instruction: {
                    parts: [
                        {
                            text: "Você é um assistente médico. Analise a transcrição de uma visita e extraia EXATAMENTE 3 campos em JSON válido.\n\n" +
