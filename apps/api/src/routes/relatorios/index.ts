...
function buildWhereDatas(filtros: z.infer<typeof filtrosSchema>) {
...
async function executarVisitas(
  orgId: string,
  ...
  const rows = await prisma.visita.findMany({
    where: {
      ...
      ...(filtros.estagios?.length && {
        profissional: { estagioPipeline: { in: filtros.estagios as any } },
      }),
      ...(filtros.potenciais?.length && {
        profissional: { potencial: { in: filtros.potenciais as any } },
      }),
...
async function executarProfissionais(
  orgId: string,
  config: z.infer<typeof configSchema>,
) {
  const { filtros = {} } = config;
  const rows = await prisma.profissional.findMany({
    where: {
      ...
      deletedAt: null,
      ...(filtros.estagios?.length && {
        estagioPipeline: { in: filtros.estagios as any },
      }),
      ...(filtros.potenciais?.length && {
        potencial: { in: filtros.potenciais as any },
      }),
      ...(filtros.especialidadeId && {
...
async function executarPipeline(
  ...
  const logs = await prisma.estagioLog.findMany({
    where: {
      ...
      ...(filtros.estagios?.length && {
        estagioNovo: { in: filtros.estagios as any },
      }),
      ...(filtros.potenciais?.length && {
        profissional: { potencial: { in: filtros.potenciais as any } },
      }),
...
function toCSV(colunas: string[], linhas: Record<string, unknown>[]): string {
...
async function executarRelatorio(
  tipo: z.infer<typeof executarSchema>["tipo"],
  orgId: string,
  config: z.infer<typeof configSchema>,
): Promise<Record<string, unknown>[]> {
  if (tipo === "visitas") {
    return (await executarVisitas(orgId, config)) as Record<string, unknown>[];
  }
  if (tipo === "profissionais") {
    return (await executarProfissionais(orgId, config)) as Record<
      string,
      unknown
    >[];
  }
  return (await executarPipeline(orgId, config)) as Record<string, unknown>[];
}
...
export default async function relatoriosRoutes(
  ...
  app.get("/configuracoes", async (request, reply) => {
    ...
    const rows = await prisma.$queryRaw<RelatorioSalvoRow[]>`
  ...
  app.post("/executar", async (request, reply) => {
    ...
    const colunas = body.config.colunas.filter((c) =>
      (permitidas as readonly string[]).includes(c),
    );
  ...
  app.get("/exportar", async (request, reply) => {
    ...
    const colunas = config.colunas.filter((c) =>
      (permitidas as readonly string[]).includes(c),
    );
    ...
    const titulo = `Relatório — ${
      query.tipo.charAt(0).toUpperCase() + query.tipo.slice(1)
    }`;
