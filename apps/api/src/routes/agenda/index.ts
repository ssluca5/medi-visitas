...
// ────────────────────────────────────────────────
// Fórmula de Sugestões — exatamente como especificado
// ────────────────────────────────────────────────
interface ProfissionalComVisita {
  id: string;
  nome: string;
  potencial: string;
...
function calcularPontuacao(
  ...
  const pesoPotencial: Record<string, number> = {
    ESTRATEGICO: 40,
    ALTO: 30,
    MEDIO: 20,
    BAIXO: 10,
...
const agendaRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/sugestoes", async (request, reply) => {
    ...
    const idsJaAgendados = [
      ...new Set(
        jaAgendados.map((a: { profissionalId: string }) => a.profissionalId),
      ),
    ];
